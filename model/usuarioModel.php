<?php
session_start();
function inserirUsuario($conn,$nomeusu,$emailusu,$foneusu,$tipousu,$cpfusu,$cepusu,$numusu,$compleusu,$senhausu,$pinusu){
   
    $salto = ['cost'=>8];
    
    $senhaCrip = password_hash($senhausu,PASSWORD_BCRYPT,$salto );

    $query = "INSERT INTO `tbusuario` (`idusu`, `nomeusu`, `emailusu`, `foneusu`, 
    `tipousu`, `cpfusu`, `cepusu`, `numusu`, `compleusu`,`senhausu`,`pinusu`) VALUES (NULL, 
    '{$nomeusu}', '{$emailusu}', '{$foneusu}', '{$tipousu}',
     '{$cpfusu}', '{$cepusu}', '{$numusu}', '{$compleusu}','{$senhaCrip}', '{$pinusu}');";
...
function alterarUsuario ($conn,$codigousu,$nomeusu,$emailusu,$foneusu,$tipousu,$cpfusu,$cepusu,$numusu,$compleusu){
...
// função para não acessar diretamente
function UsarAcesso(){
